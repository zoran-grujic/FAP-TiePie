...
class MyUi(Ui_MainWindow):
    ...
    niTask = False  # object for NI USB-6002
    ...
    def setupUi(self, MainWindow):
        ...
        self.doubleSpinBox_setBiasValue.valueChanged.connect(self.setBiasVoltage)
        ...
        self.checkBox_ActivateEOMstab.setStyleSheet("QCheckBox::indicator { width: 70; height: 70;}")
    ...
    def stabilizeProbeLevel(self):
        ...
        if self.checkBox_ActivateEOMstab.isChecked():
            ...
            tolerancePercent = self.doubleSpinBox_ProbeValueTolerance.value()
            if np.abs(error) < probeValue * tolerancePercent/100 and self.checkBox_useTolerance.isChecked():
                # no correction
                return
            correction = error * self.doubleSpinBox_ProbeLevelCorrectionFactor.value()/1000
            old = self.doubleSpinBox_setBiasValue.value()
            biasValue = old + correction
            if biasValue > 10:
                biasValue = 10
            if biasValue < -10:
                biasValue = -10
            self.doubleSpinBox_setBiasValue.setValue(biasValue)

    def setBiasVoltage(self):
        if self.niTask is False:
            try:
                self.niTask = nidaqmx.Task()
                self.niTask.ao_channels.add_ao_voltage_chan("Dev1/ao0")
            except Exception() as e:
                print("Error connecting to the NI USB device:")
                print(e)
                return

        self.niTask.write(self.doubleSpinBox_setBiasValue.value(), auto_start=True)
    ...
    def plotSCP(self):
        ...
        # stabilize probe level
        self.stabilizeProbeLevel()
    ...
    def fit(self):
        ...
        filRange = 1e3
        filterLength = 1001  # must be odd number!!!!!!!!!!!!!!!!!!!!
        pass_zero = False
        ...
        tperf = time.perf_counter() - tstart
        print(f"FIT time {tperf=} s")
